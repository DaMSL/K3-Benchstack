...
/* Hive on Spark */
/* Deprecated: TODO cleanup/revisit */
object TPCHQuery11 {
  def main(args: Array[String]) {
    val sc = Common.sc
    val sqlContext = Common.sqlContext
...
object TPCHQuery18 {
  def main(args: Array[String]) {
    ...
    val l_agg : SchemaRDD = lineitem.groupBy('l_orderkey)('l_orderkey as 'la_orderkey, Sum('l_quantity) as 'sum)
                            .where('sum > 300)
    val join1 = orders.join(customer)
                      .where('o_custkey === 'c_custkey)
    val join2 = lineitem.join(join1)
                        .where('l_orderkey === 'o_orderkey)
    val join3 = l_agg.join(join2)
                     .where('la_orderkey === 'o_orderkey)
...
object TPCHQuery22 {
  def main(args: Array[String]) {
    ...
    val r = customer.where('c_phone)((p : String) => codes.contains(p.substring(0,2)))
                    .where('c_acctbal > 0.0)
    ...
    val res = customer.where('c_phone)((p : String) => codes.contains(p.substring(0,2)))
                      .where('c_acctbal > avg_bal)
                      .join(orders, org.apache.spark.sql.catalyst.plans.LeftOuter, Some('o_custkey === 'c_custkey))
                      .filter(r => r.isNullAt(r.length - 1))
