/* Schemas and File Paths for select TPCH Queries */
...
package tpch.files {
  ...
  object TPCHFiles {
    ...
    /* SparkSQL RDD Creation */
    def getLineitem(sc: SparkContext, sf: String) = {
      val csv = sc.textFile(lineitemPath(sf)).map(_.split("\\|"))
      csv.map(r => Lineitem(
                     r(0).toInt,
                     r(1).toInt,
                     r(2).toInt, 
    ...
    def getPartsupp(sc: SparkContext, sf: String) = {
      val csv = sc.textFile(partsuppPath(sf)).map(_.split("\\|"))
      csv.map(r => Partsupp(
                     r(0).toInt,
                     r(1).toInt,
                     r(2).toInt, 
    ...
    def getNation(sc: SparkContext, sf: String) = {
      val csv = sc.textFile(nationPath(sf)).map(_.split("\\|"))
      csv.map(r => Nation(
                     r(0).toInt,
                     r(1),
                     r(2).toInt, 
    ...
    def getSupplier(sc: SparkContext, sf: String) = {
      val csv = sc.textFile(supplierPath(sf)).map(_.split("\\|"))
      csv.map(r => Supplier(
                     r(0).toInt,
                     r(1),
                     r(2), 
    ...
    def getCustomer(sc: SparkContext, sf: String) = {
      val csv = sc.textFile(customerPath(sf)).map(_.split("\\|"))
      csv.map(r => Customer(
                     r(0).toInt,
                     r(1),
                     r(2), 
    ...
    def getOrders(sc: SparkContext, sf: String) = {
      val csv = sc.textFile(ordersPath(sf)).map(_.split("\\|"))
      csv.map(r => Order(
                     r(0).toInt,
                     r(1).toInt ,
                     r(2), 
    ...
    /* HiveSQL on Spark RDD caching. For queries that SparkSQL does not support */
    def cacheLineitemHive(sqlContext: HiveContext, sf: String) = {
      val lineitemHivePath = lineitemPath(sf).init
    ...
    def cacheOrdersHive(sqlContext: HiveContext, sf: String) = {
      val ordersHivePath = ordersPath(sf).init
    ...
    def cacheCustomerHive(sqlContext: HiveContext, sf: String) = {
      val customerHivePath = customerPath(sf).init 
    ...
    def cacheSupplierHive(sqlContext: HiveContext, sf: String) = {
      val supplierHivePath = supplierPath(sf).init 
    ...
    def cacheRegionHive(sqlContext: HiveContext, sf: String) = {
      val regionHivePath = regionPath(sf).init
    ...
    def cacheNationHive(sqlContext: HiveContext, sf: String) = {
      val nationHivePath = nationPath(sf).init 
    ...
    def cachePartsuppHive(sqlContext: HiveContext, sf: String) = {
      val partsuppHivePath = partsuppPath(sf).init 
