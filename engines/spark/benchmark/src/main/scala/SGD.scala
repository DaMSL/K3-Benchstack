...
object SGD {
  def main(args: Array[String]) = {
    ...
    val parsedData = intData.map(v => (v.init, v.last)).cache() // Last column is label
    ...
    (0 until params.length).foreach(i => params(i) = .1) // Initial Parameters
  ...
  /* Vector Utils: TODO fix code duplication w/ KMeans */
